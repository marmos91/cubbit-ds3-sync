...
//!
//! These tests call the exported functions directly via Rust (not through
//! extern "C" declarations) since the test binary links against the rlib.
...
fn test_authenticate_null_email_returns_error() {
    ...
    let result = c_exports::ds3_authenticate(
        ptr::null(),          // null email
        0,
        b"password".as_ptr(),
        8,
...
fn test_list_objects_null_handle_returns_error() {
    ...
    let result = c_exports::ds3_list_objects(
        ptr::null(),          // null S3 handle
        b"bucket".as_ptr(),
        6,
        ptr::null(),
        0,
...
fn test_free_string_null_with_nonzero_len_is_noop() {
    // Should not crash: null pointer but nonzero length
    // The implementation checks for null before dereferencing
    unsafe {
...
fn test_refresh_token_null_handle_returns_error() {
    ...
    let result = c_exports::ds3_refresh_token(ptr::null(), &mut err);
...
fn test_account_info_null_handle_returns_error() {
    ...
    let result = c_exports::ds3_account_info(
        ptr::null(),
        &mut out_json,
        &mut out_json_len,
        &mut err,
...
fn test_head_object_null_handle_returns_error() {
    ...
    let result = c_exports::ds3_head_object(
        ptr::null(),
        b"bucket".as_ptr(),
        6,
        b"key".as_ptr(),
