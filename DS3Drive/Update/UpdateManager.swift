...
final class UpdateManager {
    ...
    var updateAvailable: Bool { updateChecker.updateAvailable }
    var latestVersion: String? { updateChecker.latestVersion }
    var releaseURL: String? { updateChecker.releaseURL }
    var isChecking: Bool { updateChecker.isChecking }
    var channel: DistributionChannel { updateChecker.channel }
    var lastCheckDate: Date? { updateChecker.lastCheckDate }
    ...
    var updateMenuTitle: String {
        if let version = latestVersion {
            return String(
                format: NSLocalizedString("Update Available (%@)", comment: "Tray menu update available with version"),
                version
            )
        }
